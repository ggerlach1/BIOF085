...
```{python 05-python-learning-26, results='hide'}
...
```{python tags=c("active-Rmd"), echo = FALSE, results='hide'}
graph.render('graphs/image', view=False, format='pdf');
graph.render('graphs/image', view = False, format = 'png');
```

```{r img1,  tags=c("active-Rmd"), eval = knitr::is_html_output(), echo = FALSE}
knitr::include_graphics('graphs/image.png')
```

```{r img2,  tags=c("active-Rmd"), eval=knitr::is_latex_output(), echo = FALSE}
knitr::include_graphics('graphs/image.pdf')
```
