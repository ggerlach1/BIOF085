...
```{python 01-python-tools-ds-4}
z_array.sum()
```

```{python 01-python-tools-ds-5}
z_array.min()
```

```{python 01-python-tools-ds-6}
z_array.max()
```
...
```{python 01-python-tools-ds-8}
np.array(['a','b','c'])
```

```{python 01-python-tools-ds-9}
np.array([1,2,3,6,8,29])
```

But, what if we provide a heterogeneous list?

```{python 01-python-tools-ds-10}
y = [1,3,'a']
np.array(y)
```
...
```{python 01-python-tools-ds-25}
A.shape
```

The total number of elements are given by `size`. 

```{python 01-python-tools-ds-26}
A.size
```
...
```{python 01-python-tools-ds-33}
A = rng.randn(3,5)
A
```

```{python 01-python-tools-ds-34}
A + 10
```

We can also add and multiply arrays __element-wise__ as long as they are the same shape.

```{python 01-python-tools-ds-35}
B = rng.randint(0,10, (3,5))
B
```

```{python 01-python-tools-ds-36}
A + B
```

```{python 01-python-tools-ds-37}
A * B
```
...
```{python 01-python-tools-ds-38}
A @ np.transpose(B)
```

```{python 01-python-tools-ds-39}
np.transpose(A) @ B
```
...
```{python 01-python-tools-ds-40}
D = rng.randint(0,5, (4,4))
D
```

```{python 01-python-tools-ds-41}
D.reshape(8,2)
```

```{python 01-python-tools-ds-42}
D.reshape(1,16)
```

This can also be used to cast a vector into a matrix. 

```{python 01-python-tools-ds-43}
e = np.arange(20)
E = e.reshape(5,4)
E
```
...
```{python 01-python-tools-ds-48}
A.min(axis = 0)
```

```{python 01-python-tools-ds-49}
A.max(axis = 0)
```

We can also find the **position** where the minimum and maximum values occur.

```{python 01-python-tools-ds-50}
A.argmin(axis=0)
```

```{python 01-python-tools-ds-51}
A.argmax(axis=0)
```
...
```{python 01-python-tools-ds-52}
a = rng.randint(0,10, 8)
a
```

```{python 01-python-tools-ds-53}
np.sort(a)
```

```{python 01-python-tools-ds-54}
np.argsort(a)
```

```{python 01-python-tools-ds-55}
a[np.argsort(a)]
```

`np.argsort` can also help you find the 2nd smallest or 3rd largest value in an array, too.

```{python 01-python-tools-ds-56}
ind_2nd_smallest = np.argsort(a)[1]
a[ind_2nd_smallest]
```

```{python 01-python-tools-ds-57}
...
```{python 01-python-tools-ds-61}
np.hstack((A,B))
```

```{python 01-python-tools-ds-62}
np.vstack((A,B))
```
...
```{python 01-python-tools-ds-66}
A[0,:]
```

```{python 01-python-tools-ds-67}
A1 = A
A1[0,0] = 4
A[0,0]
```
...
```{python 01-python-tools-ds-70}
A = rng.randint(0,5, (5,1))
A
```

```{python 01-python-tools-ds-71}
A.flatten()
```

```{python 01-python-tools-ds-72}
A.ravel()
```
...
```{python 01-python-tools-ds-73}
A = rng.normal(0,1,(4,5))
B = rng.normal(0,1,5)
```

```{python 01-python-tools-ds-74}
A.shape
```

```{python 01-python-tools-ds-75}
B.shape
```

```{python 01-python-tools-ds-76}
A - B
```
...
```{python 01-python-tools-ds-77}
B[np.newaxis,:].shape
```

```{python 01-python-tools-ds-78}
B[:,np.newaxis].shape
```
...
```{python 01-python-tools-ds-80}
d.shape
```

```{python 01-python-tools-ds-81}
d[np.newaxis,:,:]
```

creates a 3-d array with the first dimension being of length 1

```{python 01-python-tools-ds-82}
d[np.newaxis,:,:].shape
```

```{python 01-python-tools-ds-83}
d[:, np.newaxis,:]
```

creates a 3-d array with the 2nd dimension being of length 1

```{python 01-python-tools-ds-84}
d[:,np.newaxis,:].shape
```
...
```{python 01-python-tools-ds-90}
dist_sq
```

```{python 01-python-tools-ds-91}
dist_sq.shape
```

```{python 01-python-tools-ds-92}
dist_sq.diagonal()
```
